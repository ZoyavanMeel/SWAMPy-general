...
class ArtIllumina:
    ...
    def run(self, amplicons, n_reads):
        ...
        shuffle_fastq_file(join(self.temp, "tmp.sms.all_files_unshuffled1.fastq"), join(
            self.outpath, f"{self.output_filename_prefix}_R1.fastq"), join(self.temp, "tmp.sms.random_data"))
        shuffle_fastq_file(join(self.temp, "tmp.sms.all_files_unshuffled2.fastq"), join(
            self.outpath, f"{self.output_filename_prefix}_R2.fastq"), join(self.temp, "tmp.sms.random_data"))
